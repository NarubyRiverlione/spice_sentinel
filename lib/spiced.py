...
class spiceDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some spiced methods
    ...
    @classmethod
    def from_spice_conf(self, spice_dot_conf):
        from spice_config import spiceConfig
        config_text = spiceConfig.slurp_config_file(spice_dot_conf)
        creds = spiceConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_current_masternode_vin(self):
        from spicelib import parse_masternode_status_vin
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = spicelib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = spicelib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = spicelib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_spiced_proto_version_with_sentinel_ping)
