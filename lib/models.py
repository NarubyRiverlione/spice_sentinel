...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, spiced):
        golist = spiced.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_spiced(spiced, item)
    ...
    @classmethod
    def import_gobject_from_spiced(self, spiced, rec):
        import decimal
        import spicelib
        import inflection
        ...
        # shim/spiced conversion
        object_hex = spicelib.SHIM_deserialise_from_spiced(object_hex)
        objects = spicelib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from spiced...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from spiced, with every run
        ...
        # get/create, then sync payment amounts, etc. from spiced - spiced is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(spiced)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from spiced! %s" % e)
            govobj.vote_delete(spiced)
            return (govobj, None)
    ...
    def vote_delete(self, spiced):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(spiced, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, spiced, signal, outcome):
        import spicelib
        ...
        output = spiced.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = spicelib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(spiced, signal)

    def sync_network_vote(self, spiced, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = spiced.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = spiceD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 spice addr, non-multisig
            if not spicelib.is_valid_spice_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid spice address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (spicelib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/spiceDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import spicelib
        obj_data = spicelib.SHIM_serialise_for_spiced(self.serialise())
    ...
    def prepare(self, spiced):
        try:
            object_hash = spiced.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = spiceD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not spicelib.is_valid_spice_address(addr, config.network):
    ...
    def hash(self):
        import spicelib
        return spicelib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = spiceD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, spiced):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - spiced.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, spiced):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - spiced.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, spiced):
        now = int(time.time())
        return (self.created_at < (now - spiced.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, spiced):
        if self.is_expired(spiced):
            return False

        return True

    def is_deletable(self, spiced):
        if self.is_expired(spiced):
            return True
