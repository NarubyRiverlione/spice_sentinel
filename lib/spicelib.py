...
def is_valid_spice_address(address, network='mainnet'):
    ...
    spice_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid spice address
        return False

    if (address_version != spice_version):
        return False
...
# shims 'til we can fix the spiced side
def SHIM_serialise_for_spiced(sentinel_hex):
    from models import spiceD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = spiceD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in spiced
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # spiced expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    spiced_hex = serialise(obj)
    return spiced_hex


# shims 'til we can fix the spiced side
def SHIM_deserialise_from_spiced(spiced_hex):
    from models import spiceD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(spiced_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('spice.conf')
