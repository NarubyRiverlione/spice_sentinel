...
def has_spice_conf():
    ...
    valid_spice_conf = False

    # ensure spice_conf exists & readable
    #
    # if not, print a message stating that spice Core must be installed and
    # configured, including JSONRPC access in spice.conf
    try:
        f = io.open(config.spice_conf)
        valid_spice_conf = True
    except IOError as e:
        print(e)

    return valid_spice_conf
...
def main():
    ...
    if not has_spice_conf():
        print("spiceCore must be installed and configured, including JSONRPC access in spice.conf")
        sys.exit(1)
